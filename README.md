...
| `scripts/sync-ownership-files.sh <repo> [--apply]` | Writes both generated ownership files (`CODEOWNERS`, `COMPONENT_OWNERS.md`) into that repo's local sibling clone. **Defaults to dry-run**, showing a unified diff per file. Local only: never commits, pushes, or calls GitHub, so the target repo's own PR review still gates the change |
