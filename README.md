...
| `scripts/sync-ownership-files.sh <repo> [--apply]` | Writes everything cnpg-infra generates for a repo into its local sibling clone: `CODEOWNERS`, `COMPONENT_OWNERS.md`, `CONTRIBUTORS.md` where it has contributors, and a copy of each org-wide issue template from `cloudnative-pg/.github` **if** the repo has an `ISSUE_TEMPLATE` folder of its own (GitHub stops serving org defaults to those repos, silently). **Defaults to dry-run**, showing a unified diff per file. Local only: never commits, pushes, or calls GitHub, so the target repo's own PR review still gates the change |
